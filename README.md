...
TODO: scientist.ResultError only tracks the behavior name, not the experiment name.

```go
experiment := Experiment("widget-permissions")
experiment.RaiseErrors(func(errors ...scientist.ResultError) {
  for _, resErr := range errors {
    errortracker.Track(resErr.Err, "science failure in %s: %s", resErr.BehaviorName, resErr.Operation)
  }
})
```
